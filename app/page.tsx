...
import { getLessonSessions, getTodayQuiz } from "@/lib/quiz-repository";

export default async function HomePage() {
  const [todayQuiz, lessonSessions] = await Promise.all([
    getTodayQuiz(),
    getLessonSessions(),
  ]);
  ...
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col px-5 pb-12 pt-8 sm:max-w-lg">
      ...
      <section className="mb-8 rounded-3xl bg-gradient-to-br from-brand-400 to-brand-500 p-6 text-white shadow-lg shadow-brand-200">
        ...
        <p className="mt-1 text-xl font-bold">{todayQuiz.unitLabel} 오늘의 복습</p>

        <div className="mt-4 flex items-center gap-4 text-sm text-brand-50">
          <span>📝 {todayQuiz.questionCount}문제</span>
          <span>⏱ 약 {todayQuiz.estimatedMinutes}분</span>
        </div>

        <Link
          href={`/quiz/${todayQuiz.quizId}`}
