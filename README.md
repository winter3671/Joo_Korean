...
- **Supabase 연결 + Vercel 배포 + 마이그레이션·seed SQL 실행 완료**. 학생 화면의 콘텐츠(퀴즈/문제/선택지)는 실제 Supabase DB를 읽고, 조회에 실패하면 자동으로 `lib/mock-data.ts` 의 mock 데이터로 대체됩니다 — 아래 "Supabase에 콘텐츠 이관하기" 참고. 교사 인증·문제 CRUD·학습 기록은 아직 DB 연동 전(Phase 2의 다음 단계).
...
- **최종 목표: 관리자 화면에서 문제 데이터를 업로드하면 자동으로 Supabase에 반영** — 지금은 md 작성 → 스크립트 실행(개발 환경 필요) → SQL을 Supabase에 직접 실행, 이렇게 3단계를 거쳐야 해서 코드를 모르면 콘텐츠를 추가할 수 없음. `/admin/questions/new`(현재 저장 미동작) 같은 화면에서 비개발자도 문제를 등록·수정할 수 있고, 그게 바로 DB에 반영되는 구조로 바꾸는 게 목표. 지금 당장 급한 작업은 아니라 뒤로 미룸.
- 3차시 이후 문제은행 추가 (아래 워크플로우 반복 → `scripts/generate-seed-sql.mjs` 도 다시 실행해서 seed SQL 갱신 → Supabase에서 재실행)
